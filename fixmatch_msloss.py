...
def main():
    ...
    for epoch in range(epoch + 1, cfg['epochs']):
        ...
        for i, ((img_x, mask_x),
            ...
            loss_x_list, loss_u_s_list = [], []
            # conf_u_w_list = []

            preds_scales = model(torch.cat((img_x, img_u_w)))
            preds_scales = [pred.split([num_lb, num_ulb]) for pred in preds_scales]
            pred_x_scales = [pred[0] for pred in preds_scales]
            pred_u_w_scales = [pred[1].detach() for pred in preds_scales]
            pred_u_s_scales = model(img_u_s)
            ...
            j = 0
            for pred_x, pred_u_s, in zip(pred_x_scales,
                                         pred_u_s_scales):
                if (j != len(pred_x_scales) - 1):  # if not the last one. eg: the side out
                    gama = 0.1 / (len(pred_x_scales) - 1)
                else:
                    gama = 0.9
                j = j + 1
                ...
                loss_u_s_list.append(loss_u_s.sum() / (ignore_mask_cutmixed != 255).sum().item() * gama)

            loss_x = sum(loss_x_list)
            loss_u_s = sum(loss_u_s_list)
            ...
            if (i % (len(trainloader_u) // 8) == 0) and (rank == 0):
                ...
                print_info = 'Iters: {:}, Total loss: {:.3f}, \n' + \
                             '                                    ' + \
                             'Loss x: {:.3f}, Loss s: {:.3f}, Mask ratio: {:.3f}, \n' + \
                             '                                    ' + \
                             'time elapsed: {}, time left: {}'
                logger.info(print_info.format(i, total_loss.avg, total_loss_x.avg,
